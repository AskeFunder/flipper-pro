...
const migration = `
-- Drop table if exists and recreate with correct types
...
(async () => {
  try {
    ...
    console.log("✅ Canonical items table migrated successfully");
  } catch (err) {
    console.error("❌ Failed to migrate canonical table:", err);
  } finally {
    await db.end();
  }
})();
