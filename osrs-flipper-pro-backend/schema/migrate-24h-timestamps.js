/**
 * Migration script to add 24 hour offset to all price_24h timestamps
 * This makes timestamps represent the END of the 24-hour period instead of the START
 */
...
async function migrate24hTimestamps() {
    console.log('='.repeat(80));
    console.log('🔄 MIGRATING price_24h TIMESTAMPS (+24 hour offset)');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        ...
        // First, check if there are any conflicts (old timestamp + 86400 might conflict with existing)
        console.log('📊 Checking for potential conflicts...');
        const conflictCheck = await db.query(`
            SELECT COUNT(*) as conflict_count
            FROM price_24h p1
            INNER JOIN price_24h p2 ON p1.item_id = p2.item_id 
                AND p1.timestamp + 86400 = p2.timestamp
                AND p1.id != p2.id
        `);
        ...
        if (conflictCount > 0) {
            ...
            console.log('   This means some timestamps + 86400 already exist.');
        ...
        const countResult = await db.query('SELECT COUNT(*) as total FROM price_24h');
        const totalRows = parseInt(countResult.rows[0].total);
        console.log(`📊 Total rows in price_24h: ${totalRows}`);
        console.log();
        ...
        await db.query(`
            CREATE TEMP TABLE price_24h_migrated AS
            SELECT 
                item_id,
                timestamp + 86400 AS timestamp,  -- Add 24 hour offset
                avg_high,
                avg_low,
                low_volume,
                high_volume
        ...
        const deleteResult = await db.query('DELETE FROM price_24h');
        console.log(`✅ Deleted ${deleteResult.rowCount} rows`);
        console.log();
        
        console.log('📝 Step 4: Inserting migrated data...');
        const insertResult = await db.query(`
            INSERT INTO price_24h (item_id, timestamp, avg_high, avg_low, low_volume, high_volume)
        ...
        const verifyResult = await db.query('SELECT COUNT(*) as total FROM price_24h');
...
// Run migration
migrate24hTimestamps().catch(console.error);
