/**
 * Migration script to add 6 hour offset to all price_6h timestamps
 * This makes timestamps represent the END of the 6-hour period instead of the START
 */
...
async function migrate6hTimestamps() {
    console.log('='.repeat(80));
    console.log('🔄 MIGRATING price_6h TIMESTAMPS (+6 hour offset)');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        ...
        // First, check if there are any conflicts (old timestamp + 21600 might conflict with existing)
        console.log('📊 Checking for potential conflicts...');
        const conflictCheck = await db.query(`
            SELECT COUNT(*) as conflict_count
            FROM price_6h p1
            INNER JOIN price_6h p2 ON p1.item_id = p2.item_id 
                AND p1.timestamp + 21600 = p2.timestamp
                AND p1.id != p2.id
        `);
        ...
        if (conflictCount > 0) {
            ...
            console.log('   This means some timestamps + 21600 already exist.');
        ...
        const countResult = await db.query('SELECT COUNT(*) as total FROM price_6h');
        const totalRows = parseInt(countResult.rows[0].total);
        console.log(`📊 Total rows in price_6h: ${totalRows}`);
        console.log();
        ...
        await db.query(`
            CREATE TEMP TABLE price_6h_migrated AS
            SELECT 
                item_id,
                timestamp + 21600 AS timestamp,  -- Add 6 hour offset
                avg_high,
                avg_low,
                low_volume,
                high_volume
        ...
        const deleteResult = await db.query('DELETE FROM price_6h');
        console.log(`✅ Deleted ${deleteResult.rowCount} rows`);
        console.log();
        
        console.log('📝 Step 4: Inserting migrated data...');
        const insertResult = await db.query(`
            INSERT INTO price_6h (item_id, timestamp, avg_high, avg_low, low_volume, high_volume)
        ...
        const verifyResult = await db.query('SELECT COUNT(*) as total FROM price_6h');
...
// Run migration
migrate6hTimestamps().catch(console.error);
