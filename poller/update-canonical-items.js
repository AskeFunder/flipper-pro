...
async function calculateBatchTrendsWithCaching(itemIds, now) {
    ...
    const latestTimestamps = await db.query(`
        SELECT 'latest_5m' AS type, item_id, timestamp AS latest_ts, 
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'latest_6h' AS type, item_id, timestamp AS latest_ts,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'first_1h' AS type, item_id, timestamp AS first_ts,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'latest_24h' AS type, item_id, timestamp AS latest_ts,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
    ...
    for (const row of latestTimestamps.rows) {
        // Safeguard: parseFloat(null) returns NaN, so check for null first
        const midValue = row.mid != null ? parseFloat(row.mid) : null;
        const data = { timestamp: row.latest_ts || row.first_ts, mid: midValue };
    ...
    const allPreviousData = await db.query(`
        ...
        SELECT 'prev_5m' AS type, l.item_id,
            CASE 
        ...
        CROSS JOIN LATERAL (
            ...
            WHERE item_id = l.item_id 
              AND timestamp >= l.latest_ts - 90000  -- 25 hours ago (buffer)
              AND timestamp <= l.latest_ts - 82800  -- 23 hours ago (buffer)
              AND ABS(timestamp - (l.latest_ts - 86400)) <= 3600  -- Within 1 hour tolerance of exactly 24h ago
              AND (avg_high IS NOT NULL OR avg_low IS NOT NULL)
            ORDER BY ABS(timestamp - (l.latest_ts - 86400)) ASC, timestamp DESC LIMIT 1
        ) p
        UNION ALL
    ...
    for (const row of allPreviousData.rows) {
        // Safeguard: parseFloat(null) returns NaN, so check for null first
        const mid = row.mid != null ? parseFloat(row.mid) : null;
    ...
    for (const itemId of itemIds) {
        ...
        for (const trendName of Object.keys(windows)) {
            if (trendName === '5m') {
                ...
                if (latest && prev && prev.mid !== 0 && latest.mid != null && prev.mid != null) {
                    const trendValue = parseFloat((100.0 * (latest.mid - prev.mid) / prev.mid).toFixed(2));
                    // Safeguard: Check for NaN or Infinity before storing
                    if (isFinite(trendValue)) {
                        trends.trend_5m = trendValue;
                    } else {
...
async function calculateBatchTrends(itemIds, now) {
    ...
    const allLatestData = await db.query(`
        SELECT 'latest_5m' AS type, item_id, timestamp,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'latest_6h' AS type, item_id, timestamp,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'first_1h' AS type, item_id, timestamp,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
        ...
        UNION ALL
        SELECT 'latest_24h' AS type, item_id, timestamp,
            CASE 
                WHEN avg_high IS NOT NULL AND avg_low IS NOT NULL THEN (avg_high + avg_low) / 2.0
                WHEN avg_high IS NOT NULL THEN avg_high
                WHEN avg_low IS NOT NULL THEN avg_low
    ...
    for (const row of allLatestData.rows) {
        // Safeguard: parseFloat(null) returns NaN, so check for null first
        const midValue = row.mid != null ? parseFloat(row.mid) : null;
        const data = { timestamp: row.timestamp, mid: midValue };
    ...
    for (const row of allPreviousData.rows) {
        // Safeguard: parseFloat(null) returns NaN, so check for null first
        // The SQL CASE already handles: (high+low)/2, or high, or low, or NULL
        const mid = row.mid != null ? parseFloat(row.mid) : null;
        if (row.type === 'prev_5m') prev5mMap.set(row.item_id, { mid });
        else if (row.type === 'prev_1h') prev1hMap.set(row.item_id, { mid });
        else if (row.type === 'prev_6h') prev6hMap.set(row.item_id, { mid });
...
async function processBatch(batch, batchNum, totalBatches, now) {
    ...
    const queryTimeout = parseInt(process.env.CANONICAL_QUERY_TIMEOUT_MS || "30000", 10); // 30s default timeout
    
    // Get a dedicated client from the pool for final INSERT/UPDATE/DELETE operations
    const client = await db.connect();
    let clientReleased = false;
    
    try {
        // Set query timeout on the client to prevent hanging queries
        await client.query(`SET statement_timeout = ${queryTimeout}`);
        
        // Calculate trends for entire batch (uses pool connections - fine for reads)
        ...
        // These queries use pool connections - statement_timeout is set per-connection in db.js
        // No transaction needed for reads - they're idempotent
        ...
        const [
                    ...
                    // 1. Fetch latest price_instants for the batch (high/low prices)
                    // price_instants has separate rows for 'high' and 'low' types, so we need to pivot
                    db.query(`
                        SELECT 
                            item_id,
                    ...
                    db.query(`
                        ...
                        ORDER BY item_id, timestamp DESC
                    `, [itemIds]),
                    // 3. Combined: Volumes 1h, 6h, 24h from price_5m (single query with conditional aggregation)
                    db.query(`
                        SELECT 
                            item_id,
                    ...
                    db.query(`
                        ...
                        GROUP BY item_id
                    `, [itemIds, now - 2592000]),
                    // 6. Combined: Volumes 3m, 1y from price_24h (single query with conditional aggregation)
                    db.query(`
                        SELECT 
                            item_id,
                    ...
                    // 7. Fetch all prices from aggregated tables - OPTIMIZED: Combined queries where possible
        ...
        // OPTIMIZED: Pre-allocate Map and single-pass processing for prices from price_instants
        const pricesByItem = new Map();
        for (const itemId of itemIds) {
            pricesByItem.set(itemId, {});
        }
        // Single-pass updates from price_instants
        for (const row of priceInstantsRows.rows) {
        ...
        const [
            ...
            bsrCombined24hRows  // Combined: 3m, 1y from price_24h
                    ...
                    // NOTE: WHERE clause must use $5 (24h ago) to include all data needed for 24h calculation
                    db.query(`
                        SELECT 
                            item_id,
                            CASE 
                            ...
                            END AS ratio_24h
                        FROM price_5m
                        WHERE item_id = ANY($1) AND timestamp >= $5
                        GROUP BY item_id
        ...
        for (const itemId of itemIds) {
            ...
            values.push([
                ...
                (trends.trend_5m != null && isFinite(trends.trend_5m)) ? trends.trend_5m : null,
                (trends.trend_1h != null && isFinite(trends.trend_1h)) ? trends.trend_1h : null,
                (trends.trend_6h != null && isFinite(trends.trend_6h)) ? trends.trend_6h : null,
                (trends.trend_24h != null && isFinite(trends.trend_24h)) ? trends.trend_24h : null,
                (trends.trend_1w != null && isFinite(trends.trend_1w)) ? trends.trend_1w : null,
                (trends.trend_1m != null && isFinite(trends.trend_1m)) ? trends.trend_1m : null,
                (trends.trend_3m != null && isFinite(trends.trend_3m)) ? trends.trend_3m : null,
                (trends.trend_1y != null && isFinite(trends.trend_1y)) ? trends.trend_1y : null,
                now
            ]);
        }
        ...
        // Use client for INSERT/UPDATE to ensure proper connection handling
        await client.query(`
                    ...
                    ON CONFLICT (item_id) DO UPDATE SET
                        ...
                        trend_5m = COALESCE(NULLIF(EXCLUDED.trend_5m, 'NaN'::numeric), canonical_items.trend_5m),
                        trend_1h = COALESCE(NULLIF(EXCLUDED.trend_1h, 'NaN'::numeric), canonical_items.trend_1h),
                        trend_6h = COALESCE(NULLIF(EXCLUDED.trend_6h, 'NaN'::numeric), canonical_items.trend_6h),
                        trend_24h = COALESCE(NULLIF(EXCLUDED.trend_24h, 'NaN'::numeric), canonical_items.trend_24h),
                        trend_1w = COALESCE(NULLIF(EXCLUDED.trend_1w, 'NaN'::numeric), canonical_items.trend_1w),
                        trend_1m = COALESCE(NULLIF(EXCLUDED.trend_1m, 'NaN'::numeric), canonical_items.trend_1m),
                        trend_3m = COALESCE(NULLIF(EXCLUDED.trend_3m, 'NaN'::numeric), canonical_items.trend_3m),
                        trend_1y = COALESCE(NULLIF(EXCLUDED.trend_1y, 'NaN'::numeric), canonical_items.trend_1y),
        ...
        // Clear dirty_items for successfully processed items (use same client)
        await client.query(`
            DELETE FROM dirty_items
            WHERE item_id = ANY($1)
        `, [itemIds]);
        ...
        console.error(`[CANONICAL] Batch ${batchNum}/${totalBatches} failed:`, err.message);
        // Ensure client is released even on error
        if (!clientReleased) {
            try {
                client.release();
            } catch (releaseErr) {
                console.error(`[CANONICAL] Error releasing client:`, releaseErr.message);
            }
        }
        throw err;
    }
...
async function processBatchesInParallel(batches, now, maxConcurrency = 6) {
    ...
    // Reduced delay for faster processing when CPU headroom is available
    // Lower delay (50ms) allows faster batch starts while still preventing all batches from hitting at once
    const batchStartDelay = parseInt(process.env.CANONICAL_BATCH_DELAY_MS || "50", 10);
...
async function updateCanonicalItems() {
    ...
    try {
        ...
        // Clean up dirty_items for items that no longer exist in items table
        // This prevents items from being stuck in dirty_items forever
        const cleanupResult = await db.query(`
            DELETE FROM dirty_items
            WHERE item_id NOT IN (SELECT id FROM items)
        `);
        if (cleanupResult.rowCount > 0) {
            console.log(`[CANONICAL] Cleaned up ${cleanupResult.rowCount} dirty_items entries for non-existent items`);
        }
        
        // Get only items that need updates (from dirty_items queue)
        // ORDER BY touched_at ASC ensures oldest items are processed first (FIFO queue)
        ...
        if (items.length <= 50) {
            ...
            // Use larger batch sizes for maximum throughput when queue is large
            // Set to 800 as balance between throughput and CPU usage
            // Larger batches = fewer round trips, but too large = CPU saturation
            batchSize = parseInt(process.env.CANONICAL_BATCH_SIZE || "800", 10);
        }
        ...
        // Determine max concurrency - optimized to avoid CPU saturation
        // Reduced to 6 to prevent 100% CPU usage which causes slowdowns
        // Each batch uses ~2-3 connections (trends + data queries)
        // With pool size of 30, we can safely run 6 parallel batches
        // Performance: ~200-300 items/sec with 1000 batch size and parallel 6
        // Too many parallel batches = CPU saturation = slower overall performance
        const maxConcurrency = parseInt(process.env.CANONICAL_MAX_CONCURRENCY || "6", 10);
    ...
    try {
        ...
        await Promise.race([
            ...
            new Promise((resolve) => setTimeout(() => {
                console.warn("[CANONICAL] db.end() timed out, forcing exit.");
                resolve();
...
process.on("uncaughtException", async (err) => {
    console.error("[CANONICAL] Uncaught exception:", err.message);
    await cleanup();
    process.exit(1);
});

process.on("unhandledRejection", async (err) => {
    console.error("[CANONICAL] Unhandled rejection:", err);
    await cleanup();
    process.exit(1);
});
...
if (require.main === module) {
    updateCanonicalItems()
        .then(async () => {
            console.log("[CANONICAL] Closing database connections...");
            await cleanup();
            process.exit(0);
        })
        .catch(async (err) => {
